...
class NetworkRequest{
 ...
 //handle fetch data banner
  static List<BannerModal> parseBanner(String resBody){
    ...
    List<BannerModal> banners = listBanner.map((model) => BannerModal.fromJson(model)).toList();
    return banners;
  }
  ...
  static List<BookModal> parseBook(String resBody){
    var bookJson = jsonDecode(resBody);
    var listBook = bookJson['info'] as List<dynamic>;
    List<BookModal> books = listBook.map((model) => BookModal.fromJson(model)).toList();
    return books;
  }

  static Future<List<BookModal>> fetchBooks() async{
    final response = await http.get(Uri.parse('${baseUrl}/mainhome?account_id=50231'));
    if(response.statusCode == 200){
